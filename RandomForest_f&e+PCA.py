...
for component in range(2,X_train.shape[1]):
    ...
    for max_features in range(low_fea, up_fea+1,step_fea):
        ...
        for n_estimators in range(low_tree,up_tree+1,step_tree):
            ...
            rf_y_pred = rf.predict(X_val_pca)
    ...
    # 输出结果
    fp = open("D:\我的文件\learning\sem2\COMP7103 Data mining\\assignment 2\output\\output_fea_{}.txt".format(component), "a+")  # a+ 如果文件不存在就创建。存在就在文件内容的后面继续追加
    ...
    # 添加标题和轴标签
    plt.title('Accuracy for {} components'.format(component))

    plt.xlabel('Number of Features')
    plt.ylabel('Number of Estimators')

    plt.savefig('D:\我的文件\learning\sem2\COMP7103 Data mining\\assignment 2\output\\Result_fea_{}.png'.format(component))
    plt.clf()